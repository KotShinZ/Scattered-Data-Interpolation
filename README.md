...
### 指数型クリギング (Exponential Kriging)

距離に対して指数関数的に減衰するセミバリオグラムを採用し、急峻な変化も滑らかに補間します。ノイズを抑えたいが、鋭いピークや谷を保ちたいケースに向いています。

### ガウス型クリギング (Gaussian Kriging)

平方指数（ガウス）セミバリオグラムを使うことで無限回微分可能な非常に滑らかなサーフェスを生成します。緩やかに変化するフィールドを忠実に再現したいときに効果的です。

### 冪乗型クリギング (Power Kriging)

距離の冪乗を使ったセミバリオグラムにより、距離依存性を柔軟に設定できます。指数を調整することで平滑化の強さと局所性のバランスをとることができ、本実装では 1.5 次を既定値としています。

### 異方性クリギング (Anisotropic Kriging)

X/Y/Z 方向ごとに異なる距離スケールを設定し、特定の方向に伸びた構造を表現します。各軸を正規化した距離でガウス型セミバリオグラムを評価するため、流線方向や地層方向などの異方性を滑らかに再現できます。

### 2 次ドリフト付きユニバーサルクリギング (Universal Kriging with Quadratic Drift)

2 次までの多項式トレンドを先に学習し、その残差をガウス型クリギングで補間するハイブリッド手法です。緩やかな曲面と局所的な起伏を両立でき、滑らかな仕上がりになります。

### 線形クリギング (Linear Kriging)

距離に比例して増加する線形セミバリオグラムを使う最も素朴なクリギングです。遠方では早めにサチるため極端な値を抑えつつ、局所変動を忠実にトレースします。

### 三次クリギング (Cubic Kriging)

連続 2 階微分可能な三次セミバリオグラムを採用し、滑らかなカーブで補間します。曲率を保ったまま滑らかに変化させたいときに有効です。

### 有理二乗クリギング (Rational Quadratic Kriging)

距離スケールの異なるガウス過程を平均化したような有理二乗セミバリオグラムで、短距離・長距離双方の情報をバランス良く取り込むクリギングです。

### ホールエフェクトクリギング (Hole-Effect Kriging)

セミバリオグラムに正弦波を含めることで、振動的な「穴あき」構造を再現できます。周期的なパターンや波打つデータを滑らかに表現したいときに適しています。

### Matérn 3/2 クリギング (Matérn ν=3/2 Kriging)

Matérn カーネルの ν=3/2 を採用し、適度な滑らかさ (C2) と局所変化の表現力を両立するクリギングです。ガウス型よりもやや粗い表現が欲しい場合に使えます。

### Matérn 5/2 クリギング (Matérn ν=5/2 Kriging)

同じく Matérn カーネルの ν=5/2 版で、より高次の滑らかさ (C4) を提供します。滑らかさを保ちつつ細部も再現したいときに役立ちます。

### 対数型クリギング (Logarithmic Kriging)

距離の対数で増加するセミバリオグラムにより、ごく近傍の変化を強調しながら遠方では緩やかに変動を抑える手法です。ノイズを含むデータでも短距離構造を拾いやすくなります。

### コーシークリギング (Cauchy Kriging)

コーシー分布由来の重い裾を持つセミバリオグラムを使い、長距離にわたる滑らかな影響をモデリングします。緩やかに変化する地形や大気データなどに向きます。

### 安定分布クリギング (Stable Kriging)

指数 α を調整できる安定 (Stable) セミバリオグラムにより、距離に応じた平滑化の強さを連続的に制御できます。α を 1.2 前後に設定し、滑らかさと鋭さのバランスを確保しています。

### スプラインクリギング (Spline Kriging)

薄板スプラインと同様に曲率最小化の発想で設計したセミバリオグラムを使い、クリギングの枠組みで滑らかな曲面を再現します。薄板スプラインの滑らかさとクリギングの柔軟性を組み合わせた手法です。

### 修正シェパード法 (Modified Shepard)

古典的な距離逆比加重法（IDW）の欠点であるブルズアイ効果とフラットスポット（データ点上で勾配がゼロになる現象）を解消するため、各データ点周辺で局所的な二次関数を当てはめ、それらをブレンドする手法です。局所的なトレンドを捉えることで、疎なデータ間でも自然な補間が可能になります。

### 移動最小二乗法 (MLS - approximating)

各予測点ごとにガウス重み関数を用いた重み付き最小二乗法で局所多項式を当てはめる手法です。C∞級の滑らかさを持ちますが、データ点を厳密には通らない近似手法となります。ノイズを含むデータの平滑化に適しています。

### 補間型移動最小二乗法 (Interpolating MLS)

移動最小二乗法に特異重み関数（w = 1/r^4）を導入することで、データ点を厳密に通る補間特性を持たせた手法です。滑らかさと補間性を両立させ、疎なデータでも安定した結果が得られます。
...
3. ステータスが「学習が完了しました」と表示されたら「結果を計算」ボタンで predict のみを実行し、表・棒グラフと 3D サーフェスを描画します。
   ...
   「詳細設定」ボタンから開くウインドウの「元データ表示範囲」で X/Y/Z の最小・最大値を入力すると、その直方体に含まれる散布点だけが 3D グラフへ描画されます。読み込んだデータの軸範囲が初期値として自動設定され、値を変えても再学習や再計算は不要です。
...
2. 「詳細設定」ボタンを開くと、以下をまとめて変更できます。いずれの入力もデータ読み込み時の自動初期値から開始し、変更しても再学習は不要です。
   - 「変化させる軸」と残り 2 軸の固定値（データ範囲の中央値を初期値としてセット）
   - ライン解像度（サンプル数）: デフォルト 60。最大 2000 まで上げると滑らかな折れ線になり、下げると高速に描画できます（Python 側で安全な範囲に丸めます）。
   - 元データ表示範囲: X/Y/Z の最小・最大値を指定すると、その直方体に含まれる点だけを折れ線に重ねて表示します。
   - 表示レンジとズーム: X/Y 軸の表示範囲を手動入力してグラフのスケールを固定したり、拡大ビューのレンジをリセットできます。

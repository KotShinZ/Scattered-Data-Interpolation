...
3. ステータスが「学習が完了しました」と表示されたら「結果を計算」ボタンで predict のみを実行し、表・棒グラフと 3D サーフェスを描画します。
   ...
   「元データ表示範囲」パネルにある X/Y/Z の最小・最大値を入力すると、その直方体に含まれる散布点だけが 3D グラフへ描画されます。読み込んだデータの軸範囲が初期値として自動設定され、値を変えても再学習や再計算は不要です。
...
3. 「元データ表示範囲」パネルで X/Y/Z それぞれの最小値・最大値を入力すると、その直方体に含まれる元データの点だけが折れ線グラフに重ねて描画されます。初期値は読み込んだデータの軸範囲から自動計算され、値を変更しても再学習は不要です。
4. 「ライン解像度（サンプル数）」入力で折れ線のサンプリング密度を調整できます。デフォルトは 60 点ですが、最大 2000 点まで増やすと滑らかな曲線になり、逆に下げると高速に描画できます（Python 側には希望値が渡され、内部で安全な範囲に丸められます）。
5. 「ラインを計算」ボタンは学習済みモデルに対して **predict のみ** を実行し、指定した固定値での 1 次元ライン上の予測値を各補間法ごとに折れ線グラフで描画します。すべてのグラフには元データの散布図を重ねて表示するため、アップロードした値と補間曲線のズレをその場で確認できます。
6. CSV インポートや「ダミーデータを使用」「保存済みデータを読み込む」ボタンの挙動は `index.html` と同じで、データが変わるたびに再学習が走ります。

...
3. ページ上の「結果を計算」ボタンを押すと Pyodide 上で Python が実行され、表・棒グラフに加えて各補間法ごとの 3D サーフェスが表示されます。
   Pyodide は Web Worker 内で動作するため、学習中・predict 中でも UI が固まらず、スライダーやチェックボックスを操作できます。
...
4. 独自のデータを試したい場合は <code>x,y,z,value</code> 列を含む CSV ファイルをインポートしてください（ヘッダー行は任意）。
   ...
   読み込んだ CSV はブラウザの `localStorage` に保存されるため、`line.html` など別ページを開いても同じデータセットが自動で使われます。再度 CSV を読み込むか「ダミーデータを使用」を押すと保存内容も更新／削除されます。
...
1. データセットを読み込むと自動で全手法の学習が行われ、RMSE/滑らかさサマリーがテーブルに表示されます。
2. 「変化させる軸」で X/Y/Z のいずれかを選び、残り 2 軸の入力欄に固定値を設定します（データ範囲の中央値が初期値です）。
3. 「ラインを計算」ボタンは学習済みモデルに対して **predict のみ** を実行し、指定した固定値での 1 次元ライン上の予測値を各補間法ごとに折れ線グラフで描画します。
4. CSV インポートや「ダミーデータを使用」ボタンの挙動は `index.html` と同じで、データが変わるたびに再学習が走ります。
   いずれかのページで読み込んだ CSV は共有ストレージに保存され、`index.html` と `line.html` の間で自動的に引き継がれます。

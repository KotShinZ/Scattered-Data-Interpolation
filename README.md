...
3. 「ライン解像度（サンプル数）」入力で折れ線のサンプリング密度を調整できます。デフォルトは 60 点ですが、最大 2000 点まで増やすと滑らかな曲線になり、逆に下げると高速に描画できます（Python 側には希望値が渡され、内部で安全な範囲に丸められます）。
4. 「ラインを計算」ボタンは学習済みモデルに対して **predict のみ** を実行し、指定した固定値での 1 次元ライン上の予測値を各補間法ごとに折れ線グラフで描画します。すべてのグラフには元データの散布図を重ねて表示するため、アップロードした値と補間曲線のズレをその場で確認できます。
5. CSV インポートや「ダミーデータを使用」「保存済みデータを読み込む」ボタンの挙動は `index.html` と同じで、データが変わるたびに再学習が走ります。
